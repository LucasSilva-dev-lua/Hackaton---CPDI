...
local function gotoTema3()
	composer.gotoScene ("tema3", {time=800, effect="crossFade"})
end
...
function scene:create( event )
    ...
    local soc= display.newImageRect (grupoMain, "imagens/social2.png", 130, 157)
	soc.x = 250
	soc.y = 60
	...
	next.x = 280
	next.y = 525
	next:addEventListener ("tap", gotoTema3)

	local back= display.newImageRect (grupoMain, "imagens/hand.png", 640/8, 320/8)
	back.x = 40
	back.y = 525
	back.xScale = -1
	back:addEventListener ("tap", gotoTema1)
	...
	-- voltar:addEventListener ("tap", gotoTema1)

    local voltar = display.newText (grupoMain, "Menu", 160, 525, native.systemFont, 20)
	voltar:setFillColor(0, 0, 0)
	-- voltar:addEventListener ("tap", gotoMenu)

    -- local proximo = display.newText (grupoMain, "Próximo", 280, 450, native.systemFont, 20)
	-- proximo:setFillColor(0, 0, 0)
	-- proximo:addEventListener ("tap", gotoTema3)

	-- Definindo o texto grande
    local textoGrande = "Social (Social): Engloba as práticas de relacionamento da empresa com seus empregados, fornecedores, clientes e as comunidades onde opera. Inclui aspectos como direitos humanos, condições de trabalho, diversidade e inclusão, saúde e segurança dos trabalhadores, e contribuição para o desenvolvimento social."
